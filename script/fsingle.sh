...
fclone copy "$fclone_name":{$input_id} "$fclone_name":{$link2} --drive-server-side-across-configs --fast-list --no-traverse --size-only --stats=1s --stats-one-line -P --drive-pacer-min-sleep=1ms --ignore-checksum --ignore-existing --buffer-size=50M --use-mmap --log-level="$fs_log_level" --log-file=/root/fclone_shell_bot/log/fsingle.log
done
